...
def test_valida_rilevazione_1(setup):
...
def test_valida_rilevazione_2(setup):
    with open('scripts/attrezzature.json','r') as file_attrezzature:
        attrezzature = json.load(file_attrezzature)
    ...
    with pytest.raises(Exception):
        setup.validaRilevazione(accounts[1], attrezzature[0]['IDDispositivo'],attrezzature[0]['Tipologia dispositivo'], {'from':accounts[2]})

def test_valida_rilevazione_3(setup):
    with open('scripts/attrezzature.json','r') as file_attrezzature:
        attrezzature = json.load(file_attrezzature)
    ...
    with pytest.raises(Exception):
        setup.validaRilevazione(accounts[7], attrezzature[0]['IDDispositivo'],attrezzature[0]['Tipologia dispositivo'], {'from':accounts[1]})
