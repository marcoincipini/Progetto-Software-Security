...
def main():
    ...
    while menu:
        try:
            scelta = int(
                input(
                    "Seleziona azione (0: Invia una richiesta come nuovo paziente; 1: Visualizza/accetta richieste con l'utente ASUR; altro intero: esci): "
                )
            )
            ...
            if scelta == 0:
                ...
                nuovo_ID = len(
                    accounts
                )  # l'ID dell'account è impostato in modo che sia la posizione successiva all'ultima
                ...
                while i:
                    ...
                    try:
                        nuovo_paziente = {
                            ...
                            "Latitudine": int(
                                input(
                                    "Inserisci la latitudine del nuovo paziente (42 - 43): "
                                )
                            ),
                            "Longitudine": int(
                                input(
                                    "Inserisci la longitudine del nuovo paziente (13): "
                                )
                            ),
                        }
                        # controllo che il paziente inserito sia localizzato nelle Marche (lat 42-43 lon 13)
                        if (
                            nuovo_paziente["Latitudine"] < 42
                            or nuovo_paziente["Latitudine"] > 43
                            or nuovo_paziente["Longitudine"] < 13
                            or nuovo_paziente["Longitudine"] > 13.8
                        ):
                            print(
                                "I dati geografici inseriti sono fuori dall'area di influenza del servizio."
                            )
                            print("Riprovare...")
                        else:
                            i = False
                        ...
                        print(
                            "Input non valido. Si prega di inserire un numero intero."
                        )
                ...
                accounts.add()
                print(
                    "E' stato creato un nuovo paziente: ",
                    accounts[nuovo_paziente["ID"]],
                )

                # viene inviata una richiesta dal paziente
                contratto.setRichieste(
                    accounts[nuovo_paziente["ID"]],
                    nuovo_paziente["Latitudine"],
                    nuovo_paziente["Longitudine"],
                    {"from": accounts[nuovo_paziente["ID"]]},
                )
                print(
                    "Il paziente ",
                    accounts[nuovo_paziente["ID"]],
                    " ha inviato una richiesta ADI.",
                )
            ...
            # Selezione per accettare le richieste ADI di nuovi utenti da parte dell'ASUR
                ...
                richieste = (
                    contratto.getRichieste()
                )  # ottiene tutte le richieste ancora non accettate
                if not richieste:
                    ...
                    try:
                        selezione = int(
                            input(
                                "Inserisci il numero corrispondente alla richiesta che desideri accettare: "
                            )
                        )
                        if 1 <= selezione <= len(richieste):
                            indice = selezione - 1
                            indirizzo_pz, lat_pz, lon_pz = richieste[indice]
                            contratto.setPazienteASUR(
                                indirizzo_pz,
                                lat_pz,
                                lon_pz,
                                indice,
                            ...
                            print(
                                f"Hai inserito il paziente {indirizzo_pz} al programma ADI"
                            )
                        else:
