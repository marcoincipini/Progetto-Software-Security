...
def main():
    ...
    while menu1:
        try:
            scelta = int(
                input(
                    "Seleziona azione (0: Utente medico - Visualizza/Aggiungi terapia; 1: Utente Paziente - Visualizza piano terapeutico; altro intero: esci):\n "
                )
            )
            # Utente medico
            if scelta == 0:
                medici = contratto.getMedici({"from": accounts[0]})
                for i, medico in enumerate(
                    medici
                ):  # visualizzo tutti i medici per la selezione
                    print(f"{i + 1}: {medico}")
                try:
                    selezione = int(input("Seleziona ID dell'utente medico:\n"))
                    if (
                        1 <= selezione <= len(medici)
                    ):  # se la selezione è valida consento la selezione di un paziente associato
                        print("accesso effettuato come account Medico\n")
                        indirizzo_med = medici[selezione - 1]
                        pazienti_med = contratto.getPazientiDelMedico(
                            {"from": indirizzo_med}
                        )  # ottengo tutti i pazienti associati al medico
                        for i, paziente_med in enumerate(
                            pazienti_med
                        ):  # visualizzo tutti i pazienti associati al medico
                            print(f"{i + 1}: {paziente_med}")
                        try:
                            selezione = int(
                                input("Seleziona ID del paziente: ")
                ...
                if menu2:
                    ...
                    hash_terapia = contratto.getTerapia(
                        indirizzo_paz, {"from": indirizzo_med}
                    )
                    ...
                    try:
                        selezione = int(
                            input(
                                "Seleziona azione (0: Modifica piano terapeutico; 1: Aggiungi nuova terapia; altro intero: esci):\n"
                            )
                        )
                        ...
                        if selezione == 0:
                            try:
                                id_terapia = int(
                                    input("Seleziona ID della terapia:\n")
                                ...
                                if id_terapia >= 1 and id_terapia <= len(json_terapia):
                                    ...
                                    for chiave, valore in terapia_selezionata.items():
                                        if chiave in [
                                            ...
                                            "Nome",
                                            "Cognome",
                                        ]:  # salto chiavi da non modificare
                                            pass
                                        else:
                                            nuovo_valore = input(
                                                f"Modifica elemento {chiave} (default: {valore}): "
                                            )
                                            if checkValore(
                                                nuovo_valore, chiave
                                            ):  # valida il valore inserito
                                                terapia_selezionata[chiave] = (
                                                    nuovo_valore
                                                )
                                            else:
                                                if nuovo_valore:
                                                    print(
                                                        "Valore non valido! Verrà mantenuto il valore di default.\n"
                                                    )
                        ...
                        # selezione di aggiunta nuova voce al piano terapeutico
                            ...
                            nuova_terapia["Cognome"] = temp[
                                "Cognome"
                            ]  # ID, nome, cognome sono preimpostati
                            for chiave in [
                                "Operatore",
                                "Data inizio",
                                "Data fine",
                            ...
                            json_terapia.append(
                                nuova_terapia
                            )  # aggiunge la voce al json del piano terapeutico
                            # upload su pinata e su blockchain
                            uploadTerapia(
                                contratto, json_terapia, indirizzo_paz, indirizzo_med
                            )
                    except ValueError:
            ...
            # Scelta dell'utente paziente - visualizzazione json
                ...
                try:
                    ...
                    if 1 <= selezione <= len(pazienti):
                        ...
                        indirizzo_paz = paz[0]
                        hash_terapia = contratto.getTerapia(
                            indirizzo_paz, {"from": indirizzo_paz}
                        )
